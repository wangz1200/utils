...
def download_all(date, dir_):
    jr = JmReport()
    jr.login().init()

    report = jr.new_customer_report()
    report.init()
    report.set_date(date)
    report.download(os.path.join(dir_, "CUS-D-" + date + ".txt"))

    report = jr.new_deposit_report()
    report.init()
    ...
    for inst in ("70315", "70317", "70318", "70319", ):
        report.set_inst(inst)
        report.download(os.path.join(dir_, "DEP-D-" + inst + "-" + date + ".txt"))

    jr.close()


if __name__ == "__main__":
    download_all("20190219", "D:/Desktop")
